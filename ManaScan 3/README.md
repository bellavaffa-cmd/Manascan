...
There's no Android SDK/build toolchain available in the sandbox this project
was generated in, so an APK couldn't be built and handed to you directly. Two
...
2. The included `.github/workflows/build-apk.yml` runs automatically on push
   and builds a debug APK in the cloud.
3. Open the repo's **Actions** tab → the latest run → download the
   `ManaScan-debug-apk` artifact (a zip containing `app-debug.apk`).
4. Transfer that APK to your phone (email, Drive, USB, etc.), open it, and
   allow "install from unknown sources" when prompted.

**Option B — Android Studio, build once, keep the APK**
1. Follow the "Building" steps above to open and sync the project.
2. Build menu → **Build Bundle(s) / APK(s) → Build APK(s)**.
3. Android Studio will show a "locate" link to
   `app/build/outputs/apk/debug/app-debug.apk` — copy that file to your phone
   the same way as above.

A debug APK is unsigned for the Play Store but installs and runs fine when
sideloaded directly onto your own device.
